# Documentation index

| Path | Purpose |
|------|---------|
| [architecture/ARCHITECTURE.md](architecture/ARCHITECTURE.md) | System design |
| [architecture/LAYOUT.md](architecture/LAYOUT.md) | Repository map |
| [architecture/TRANSPORT.md](architecture/TRANSPORT.md) | Transport adapter contract |
| [protocol/SPEC.md](protocol/SPEC.md) | Normative protocol |
| [protocol/COMPATIBILITY.md](protocol/COMPATIBILITY.md) | Interop statement |
| [api/API.md](api/API.md) | Public C++ API |
| [development/SDK_GUIDE.md](development/SDK_GUIDE.md) | Writing features |
| [development/TESTING.md](development/TESTING.md) | Test suites |
| [development/MIGRATION_MESHCORE.md](development/MIGRATION_MESHCORE.md) | MeshCore consumer guide |
| [development/WORKSPACE.md](development/WORKSPACE.md) | `/data/projects` layout |
| [development/MESHCORE_DEPENDENCIES.md](development/MESHCORE_DEPENDENCIES.md) | What stays in MeshCore |
| [development/EXTRACTION.md](development/EXTRACTION.md) | Extraction record |
| [DEVELOPMENT_RULES.md](DEVELOPMENT_RULES.md) | Mandatory git / config / QA / release rules |
| [protocol/COMMAND_COVERAGE.md](protocol/COMMAND_COVERAGE.md) | RC command + stress coverage |
| [development/RELEASE.md](development/RELEASE.md) | Release process |
| [homeassistant.md](homeassistant.md) | HA integration design |
| [roadmap/ROADMAP.md](roadmap/ROADMAP.md) | Future plans |
| [STATE.md](STATE.md) | Current maturity |
| [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) | Publish gate |

Root entry point: [../README.md](../README.md).
